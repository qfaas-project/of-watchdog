# QFaaS Note for of-watchdog

of-watchdog for QFaaS project
...
```shell script
./build.sh
docker tag openfaas/of-watchdog:latest-dev-x86_64 qfaas/of-watchdog:latest-dev-x86_64 
docker images
# You should find a docker image called qfaas/of-watchdog:latest-dev-x86_64
# You should also find a binary file, of-watchdog, in the project root path
```

## Major Modifications

- To get all the major modifications we made, please `diff` the following files to the original version 
    - [`main.go`](main.go)
    - [`executor/http_runner.go`](executor/http_runner.go)
- We updated the go version from `1.13` to `1.15` in `Dockerfile` to support the newer `quic-go`  
- Please check [`vendor/github.com/lucas-clemente/quic-go`](vendor/github.com/lucas-clemente/quic-go) to get all modifications we made to `quic-go`  
- [cert](cert) folder contains our self-signed certificate for prototype testing purpose
