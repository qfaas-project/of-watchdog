...
func main() {
	...
	log.Println("QFaaS Of-WatchDog Version: v21.5")

	mux := http.NewServeMux()
	mux.HandleFunc("/", metrics.InstrumentHandler(requestHandler, metrics.NewHttp()))
	mux.HandleFunc("/_/health", makeHealthHandler())
	...
	listenUntilShutdown(shutdownTimeout, s_quic, watchdogConfig.SuppressLock)
}
...
func listenUntilShutdown(shutdownTimeout time.Duration, s *http3.Server, suppressLock bool) {
	...
	go func() {
		if err := s.ListenAndServeTLS(GetCertificatePaths()); err != http.ErrServerClosed {
